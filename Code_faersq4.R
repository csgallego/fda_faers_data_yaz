...
#turn that into a histogram
hist(Yazyear$count,col=topo.colors(4))
qqnorm(Yazyear$count)

#We now look at the mean and median
mean(Yazcount)
[1] 3658.429
median(Yazcount)
[1] 1703.5
...
#I know would like to calculate the difference between the number of complaints received for Yaz/Yasmin and for second-generation pills
#I create a new y-axis that represent difference between the complaitns for Yaz and complaints for other drugs.
diff=Yazcount-Drugcount
table(diff)
Diff2=data.frame(quarter=Dquarter,count=diff)

#Let's look in terms of percentage
diffpercent=((Yazcount-Drugcount)/Yazcount)*100
Diffpercent2=data.frame(quarter=Dquarter,count=diffpercent)
plot(Diffpercent2, type="l",sub="Source: FDA Quarterly Data Files")
lines(Diffpercent2, type="l",sub="Source: FDA Quarterly Data Files")

#If we turn that into a histogram
hist(Diffpercent2$count,col=topo.colors(6))

#The normality of the distribution 
qqnorm(Diffpercent2$count)
...
#Now I'd like to compare the total of complaints for Yaz and the other drugs
TotalYaz=sum(Yazcount)
[1] 102436
TotalDrug=sum(Drugcount)
[1] 3206
